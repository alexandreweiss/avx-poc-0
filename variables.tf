...
# --- Optional: Aviatrix User VPN ---

variable "deploy_vpn" {
  description = "Deploy Aviatrix User VPN gateway with split-tunnel access to all spoke and EKS CIDRs"
  type        = bool
  default     = false
}

variable "vpn_user_email" {
  description = "Email address for the VPN user account (e.g. user@example.com). Required when deploy_vpn=true."
  type        = string
  default     = ""
}
...
variable "vpn_gw_size" {
  description = "EC2 instance size for the VPN gateway"
  type        = string
  default     = "t3.medium"
}
...
variable "eks_admin_iam_role" {
  description = "IAM role ARN granted system:masters on the EKS cluster (e.g. SSO role for the deployer). Leave empty to skip."
  type        = string
  default     = ""
}

variable "eks_service_cidr" {
  description = "EKS Kubernetes service CIDR (ClusterIP range) — added to VPN split tunnel when deploy_vpn=true"
  type        = string
